...
export const download = async (project_id: string): Promise<void> => {
  const response = await axios.post(
    `${API_URL}/arduino/projects/${project_id}/download/`
  );
...
export const uploadCodeToBoard = async (projectId: string): Promise<void> => {
  try {
    await axios.post(`${API_URL}/arduino/projects/${projectId}/download/`);
  } catch (error) {
